...
class ColorMapper {
  ...
  _gradientSpan(type, value) {
    ...
    if (type === "h") for (let i = 0; i < 7; i++) gradient.push(`hsl(${(i / 7) * 360}, 100%, 60%)`);
    else if (type === "s") for (let i = 0; i < 7; i++) gradient.push(`hsl(0, ${(i / 7) * 100}%, 60%)`);
    else if (type === "l") for (let i = 0; i < 7; i++) gradient.push(`hsl(0, 0%, ${(i / 7) * 100}%)`);
    return `
      <div>
